...
const runTests = async () => {
	...
	const browser = await puppeteer.launch(debugOpts);
	...
	test('getPopUpHash: return hash with content from narrative popup', async t => {
		const page = await getDeficencyPage(95732, browser);
		const rows = await getDeficenciesRow(page);

		let result = await getPopUpHash(rows[0], page, 95732);
		let expected = { stuff: 'here' };
		t.deepEqual(result, expected);
